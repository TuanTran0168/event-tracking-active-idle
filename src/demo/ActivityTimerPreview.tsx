...
import type { ActivityState } from "../core/activity-detector";
import { useActivityDetector } from "../core/useActivityDetector";
import { useInputMonitor, type MonitoredInputEvent } from "./useInputMonitor";

type StateLog = { id: string; state: ActivityState; timestamp: number; since: number };
...
export function ActivityTimerPreview() {
  ...
  const handleStateChange = useCallback((nextState: ActivityState, timestamp: number, since: number) => {
    const payload = {
      state: nextState,
    ...
    setStateLogs((current) => [
      { id: `${timestamp}-${nextState}`, state: nextState, timestamp, since },
      ...current,
    ].slice(0, 6));
  }, []);
        ...
        <article className="card event-card">
          ...
          {stateLogs.length === 0 ? <p className="empty-log">Chưa có transition. Mở DevTools để xem payload tracking qua console.log.</p> : (
            <ul className="event-list">
              {stateLogs.map((event) => {
                const blindFloorMs = event.timestamp - event.since;
                return (
                  <li key={event.id}>
                    <span className={`dot ${event.state}`} />
                    <span>
                      <strong>{event.state}</strong>
                      <small className="since-hint">
                        {blindFloorMs > 0
                          ? `since ${formatTime(event.since)} (trễ ${blindFloorMs}ms so với lúc phát hiện)`
                          : `since = timestamp (chuyển tức thời, không có blind floor)`}
                      </small>
                    </span>
